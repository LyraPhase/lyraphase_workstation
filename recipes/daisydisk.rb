...
template File.join(app_supportdir, 'DaisyDisk', 'License.DaisyDisk') do
  ...
  owner node['current_user']
  variables :license => node['lyraphase_workstation']['daisydisk']['license']
  not_if { node['lyraphase_workstation']['daisydisk']['license']['customer_name'].nil? || node['sprout']['daisydisk']['license']['registration_key'].nil? }
end
